...
public class GDSInputDecimalTests : TestContext
{
    [Fact]
    public void GDSInputDecimal_Parameters_Are_Set_To_Error()
    {
        ...
        var component = RenderComponent<GDSInputDecimal>(parameters =>
        {
            parameters.Add(p => p.Data, 123.45M);
        ...
        component.Instance.Data.Should().Be(123.45M);
    ...
    [Fact]
    public void GDSInputDecimal_Parameters_Are_Set_To_Validate()
    {
        RenderTree.Add<CascadingValue<EditContext>>(parameters =>
        {
            parameters.Add(p => p.Value, new EditContext(new PaymentRequest()));
